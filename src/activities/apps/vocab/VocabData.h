...
// Built-in flashcard deck: ~200 high-frequency CET-4 words with concise
// Chinese glosses. Flash-resident (constexpr); glyph coverage comes from the
// cn_apps_chars.txt scan of this directory (see chinese-build.md).
struct VocabWord {
  const char* word;
  const char* pos;
  const char* meaning;
...
constexpr VocabWord kVocabWords[] = {
    {"abandon", "v.", "抛弃；放弃"},
...
constexpr int kVocabWordCount = static_cast<int>(sizeof(kVocabWords) / sizeof(kVocabWords[0]));
