...
- **CN network apps** (`gh_release_cn` only): **Weather** (天气 — current conditions + 3-day outlook for 40 built-in Chinese cities via the free Open-Meteo API), **Daily Poem** (每日诗词 — one classical poem line per fetch, cached for offline display), and an **RSS reader** (RSS 订阅 — subscriptions in an editable `/rss-feeds.json` on the SD card, up to 20 articles per feed fetched with a streaming parser and cached on SD for offline reading). All network apps connect on demand through the shared Wi-Fi picker and power Wi-Fi down on exit. The CN build also adds **Exchange Rates** (汇率), a **Pomodoro timer** (番茄钟), CET-4 **Flashcards** (单词卡), the classic **Klotski** sliding puzzle (华容道, nine BFS-verified layouts with per-layout best records), and **三国霸业** — a turn-based Three-Kingdoms strategy game (12-city map, 20 historical generals, develop/recruit/march actions, battle resolution with wall and general bonuses, two heuristic AI rivals, and campaign save/resume).
