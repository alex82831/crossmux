...
3. **App store catalog** (generated by `sdk/dynapp/build-store.sh`):
   - `store/catalog.json` — the install catalog the on-device App Manager fetches
   - `store/*.eapp` — the prebuilt app images
   - **To modify**: edit the app's `sdk/dynapp/apps/<slug>/app.meta`
     (`name` / `version` / `note`), then run `./sdk/dynapp/build-store.sh`.
     Byte counts are read off the built files, so hand-editing the JSON is
     how a stale size ships.
   - **Commit**: both the sources *and* the regenerated `store/` output —
     unlike the headers above, these are the download artifacts users install.
