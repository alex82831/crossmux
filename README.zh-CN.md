...
- **应用管理与可安装应用（DynApp）**：新增 **应用管理**（Apps 菜单内）。以往应用全部编译进固件，安装/更新都要重新刷机并会丢失缓存；现在应用可做成独立的 `.eapp` 文件放在 SD 卡 `/apps/` 下，运行时安装、更新、卸载，**无需刷机、数据不丢**。可从**在线目录**通过 Wi‑Fi 一键安装/更新，或用「文件传输」在浏览器里把 `.eapp` 上传到 `/apps/`（复用既有的无线传书通道，无需蓝牙）。应用数据存放在 `/apps/data/<slug>/`，刷机与更新都不会清除。技术原理（ESP32‑C3 上如何从堆内存执行动态代码、地址域换算、ABI 与 SDK）见 [docs/engineering/dynapp.md](./docs/engineering/dynapp.md)；示例应用「系统监视器」「生命游戏」的源码与目录见 [`sdk/dynapp/`](./sdk/dynapp) 与 [`store/`](./store)。
