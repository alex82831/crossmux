...
**FLASHAPPS** 是 [CrossPoint Reader](https://github.com/crosspoint-reader/crosspoint-reader) 的社区 fork：在原有电子书阅读体验之上，新增了一个 Apps 应用中心（小游戏 / 小工具）、更丰富的待机表盘，以及一套完整的简体中文固件。

**版本：** FLASHAPPS 1.5.7（基于 CrossPoint Reader 1.5.0，并同步上游 `develop` 至 `eef20504`）

**运行设备：** 基于 ESP32-C3 的 Xteink [X4](https://www.xteink.com/products/xteink-x4) 与 [X3](https://www.xteink.com/products/xteink-x3)。

> 本文档面向中文用户，重点说明 FLASHAPPS 的中文相关功能与简体中文固件的编译方式。完整的英文说明请见 [README.md](./README.md)。

![FLASHAPPS 运行在 Xteink 设备上](./docs/images/cover.jpg)
...
- **中文网络应用**（仅 `gh_release_cn`）：**天气**（内置 40 个中国主要城市，免费 Open-Meteo 接口，当前天气 + 3 日预报，上下键切换城市）、**每日诗词**（每次刷新获取一句古诗词，离线显示上次内容），以及 **RSS 订阅**（订阅源保存在 SD 卡可编辑的 `/rss-feeds.json`，每源流式抓取最多 20 篇文章并缓存到 SD 卡供离线阅读，出厂预置少数派、阮一峰、36氪、Solidot、BBC 中文五个源）。联网应用均按需通过共享 Wi‑Fi 选择器联网，退出应用即关闭 Wi‑Fi。此外还有 **汇率**（八种主要货币对人民币，金额档位切换）、**番茄钟**（专注/休息循环、三种时长方案、每日统计）、**单词卡**（内置高频四级词，三盒记忆法）、**华容道**（九个经过求解器验证的经典布局、步数与最佳纪录、悔棋与中途续玩），以及 **三国霸业**——回合制三国策略游戏：十二城地图、二十名史实武将（武力/智力）、开垦/通商/筑城/征兵/出征五种内政军事行动、含城防与武将加成的战斗结算、两个启发式 AI 对手、攻占武将镇守的中立城可招揽其归顺，支持随时存档续玩。所有上述自研应用均内置“关于”页（无菜单的应用长按返回键约 1 秒打开，游戏在菜单中）。
- **应用全部改为可安装（DynApp）**：固件现在**只内置「应用管理」**与少数无法独立的基础设施（文件传输、OPDS、微信读书、AirPage、像素开关、阅读分析、待机）。以往编译进固件的**全部游戏与工具**（2048、扫雷、数独、五子棋、推箱子、华容道、计算器、木鱼、番茄钟、单词卡、三国霸业、中国象棋、天气、每日诗词、RSS、汇率、头像、抽卡等 18 个）现在都是独立的 `.eapp` 文件，用户**自行按需安装**——运行时安装/更新/卸载，**无需刷机、数据不丢**。四种安装方式：在线目录 Wi‑Fi 一键装；**「从本地安装」选择 SD 卡上的 `.eapp`**；在「文件管理」里直接打开 `.eapp`；或用「文件传输」从浏览器上传到 `/apps/`。**在线目录的服务器地址可在设备上自行配置**（默认仍为 githubusercontent，清空即恢复默认）。应用数据在 `/apps/data/<slug>/`，刷机与更新都不清除。应用的中文文字通过固件内置字库渲染（故中文版仍保留完整 CJK 覆盖）。技术原理见 [docs/engineering/dynapp.md](./docs/engineering/dynapp.md)；全部应用源码见 [`sdk/dynapp/apps/`](./sdk/dynapp/apps)，预编译与在线目录见 [`store/`](./store)。
- **音乐播放器**（可安装应用）：播放 SD 卡 `/music` 下的音乐。**说明：X3/X4 没有任何音频硬件，ESP32‑C3 也只有低功耗蓝牙（无经典蓝牙），因此本机放音与蓝牙耳机在硬件上就不可能**。可行的是 DLNA 三件套模式：本机作控制器，把歌曲通过自身的 HTTP 服务发布，由局域网里的 DLNA 音箱/电视/手机 App 直接拉流播放，音频不经过设备。支持自动发现播放设备并记住上次选择、播放/暂停/上下曲/音量/进度显示、播完自动下一首。
- **文件管理**（Apps 菜单内）：真正的全功能文件管理器——浏览 SD 卡全部文件与目录（图书馆的浏览器只列阅读器能打开的格式，所以在那里看 `/apps` 是空的），支持复制、剪切、粘贴、重命名、删除、新建文件夹、显示/隐藏隐藏文件与存储空间查看；**打开文件按类型分发**：选中 `.eapp` 即安装该应用，选中 EPUB/TXT/MD 等直接进阅读器。操作方式：上下移动、确认进入或打开、返回上一级、左键打开「此处操作」菜单、长按条目打开该项的操作菜单。
- **复古主题**（设置 → 界面主题 → 复古）：致敬 System 7 / Windows 3.x 时代的 1‑bit 界面——条纹标题栏、双线窗框、带硬阴影的斜面按钮、分段式电量格、文件夹式标签页，专为电子墨水的局部刷新优化（纯黑白几何，不用抖动填充）。
...
上游 CrossPoint 的全部能力（EPUB 2/3 渲染、多格式支持、无线传书、OPDS、OTA 等）在 FLASHAPPS 中同样可用，详见 [English README](./README.md#what-can-crosspoint-do)。

---

## 编译简体中文固件

FLASHAPPS 提供一个专用的简体中文构建环境 `gh_release_cn`，产出**仅中文**的固件：简体中文 UI + i18n、内嵌 CJK 点阵字体、面向中文的 EPUB 排版，以及中国象棋 / 微信读书等应用。全新设备首次开机即直接进入中文界面。
...
`fs_/books/`，再运行 FLASHAPPS 使用的桌面模拟器：
...
FLASHAPPS / CrossPoint Reader **与 Xteink 及任何设备厂商均无隶属关系**。
