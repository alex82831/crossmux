...
- **应用全部改为可安装（DynApp）**：固件现在**只内置「应用管理」**与少数无法独立的基础设施（文件传输、OPDS、微信读书、AirPage、像素开关、阅读分析、待机）。以往编译进固件的**全部游戏与工具**（2048、扫雷、数独、五子棋、推箱子、华容道、计算器、木鱼、番茄钟、单词卡、三国霸业、中国象棋、天气、每日诗词、RSS、汇率、头像、抽卡等 18 个）现在都是独立的 `.eapp` 文件，用户**自行按需安装**——运行时安装/更新/卸载，**无需刷机、数据不丢**。三种安装方式：在线目录 Wi‑Fi 一键装；「文件传输」浏览器上传 `.eapp` 到 `/apps/`；或直接拷进 SD 卡。应用数据在 `/apps/data/<slug>/`，刷机与更新都不清除。应用的中文文字通过固件内置字库渲染（故中文版仍保留完整 CJK 覆盖）。技术原理见 [docs/engineering/dynapp.md](./docs/engineering/dynapp.md)；全部应用源码见 [`sdk/dynapp/apps/`](./sdk/dynapp/apps)，预编译与在线目录见 [`store/`](./store)。
