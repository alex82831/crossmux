...
- **中文网络应用**（仅 `gh_release_cn`）：**天气**（内置 40 个中国主要城市，免费 Open-Meteo 接口，当前天气 + 3 日预报，上下键切换城市）、**每日诗词**（每次刷新获取一句古诗词，离线显示上次内容），以及 **RSS 订阅**（订阅源保存在 SD 卡可编辑的 `/rss-feeds.json`，每源流式抓取最多 20 篇文章并缓存到 SD 卡供离线阅读，出厂预置少数派、阮一峰、36氪、Solidot、BBC 中文五个源）。三者均按需通过共享 Wi‑Fi 选择器联网，退出应用即关闭 Wi‑Fi。
